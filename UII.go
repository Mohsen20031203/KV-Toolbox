...
func setupLastColumn(app fyne.App, rightColumnContentORG *fyne.Container) *fyne.Container {
	...
	go func() {
		...
		if err != nil {
			...
			for _, project := range jsonData.RecentProjects {
				buttonContainer := projectButton(project.Name, lastColumnContent, project.FileAddress, rightColumnContentORG)
